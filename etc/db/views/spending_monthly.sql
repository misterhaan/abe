...
select month, sum(a.amount) as amount, c.id as catid, c.name as catname, g.id as groupid, g.name as groupname
	from (select str_to_date(concat(year(posted), '-', month(posted), '-01'), '%Y-%m-%d') as month, sum(amount) as amount, category
		from transactions where splitcat=0 group by month, category
	union all select str_to_date(concat(year(t.posted), '-', month(t.posted), '-01'), '%Y-%m-%d') as month, sum(s.amount) as amount, s.category
