...
select min(a.displaydate) as displaydate, concat(a.sortdate, '-01') as datestart, last_day(concat(a.sortdate, '-01')) as dateend, sum(a.amount) as amount, c.id as catid, coalesce(c.name, '(uncategorized)') as catname, p.id as parentid, p.name as parentname
