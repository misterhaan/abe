...
class abeKeysDB {
		...
		if (isset($_POST['rootpw']) && ($rootpw = trim($_POST['rootpw']))) {
			...
			if (!$rdb->errno) {
				...
				if ($rdb->real_query('create database if not exists `' . $rdb->escape_string(abeKeysDB::NAME) . '` character set utf8mb4 collate utf8mb4_unicode_ci'))
					if ($rdb->real_query('grant all on `' . $rdb->escape_string(abeKeysDB::NAME) . '`.* to \'' . $rdb->escape_string(abeKeysDB::USER) . '\'@\'localhost\' identified by \'' . $rdb->escape_string(abeKeysDB::PASS) . '\''));  // done here!
					else
	...
	protected static function installDatabaseAction(abeAjax $ajax) {
		...
		if ($db->real_query('insert into config (structureVersion) values (' . +Version::Structure . ')')) {
			if (self::ImportBanks($db, $ajax)) {
				...
				if (!$ajax->Data->fail)
					if (self::SetDataVersion(Version::Data, $db, $ajax))
						$db->commit();
					else
	...
	private static function UpgradeDatabaseStructure(mysqli $db, abeAjax $ajax) {
		if (
			$db->config->structureVersion < StructureVersion::Bookmarks
			&& !self::UpgradeDatabaseStructureStep(StructureVersion::Bookmarks, $db, $ajax, 'tables/bookmarks')
		)
			return false;

		if (
			$db->config->structureVersion < StructureVersion::Duplicates
			&& !self::UpgradeDatabaseStructureStep(StructureVersion::Duplicates, $db, $ajax, 'routines/IsDuplicateTransaction')
		)
			return false;

		if (
			$db->config->structureVersion < StructureVersion::SummaryProcedures
			&& !self::UpgradeDatabaseStructureStep(
				StructureVersion::SummaryProcedures,
				$db,
				$ajax,
				'routines/GetMonthlyCategorySpending',
				'routines/GetYearlyCategorySpending'
		...
		if (
			$db->config->structureVersion < StructureVersion::CategoryGroups
			&& !self::UpgradeDatabaseStructureStep(
				StructureVersion::CategoryGroups,
				$db,
				$ajax,
				'tables/category_groups',
		...
		if (
			$db->config->structureVersion < StructureVersion::Funds
			&& !self::UpgradeDatabaseStructureStep(StructureVersion::Funds, $db, $ajax, 'tables/funds')
		)
			return false;

		if (
			$db->config->structureVersion < StructureVersion::Budgets
			&& !self::UpgradeDatabaseStructureStep(
				StructureVersion::Budgets,
				$db,
				$ajax,
		...
		if (
			$db->config->structureVersion < StructureVersion::MySqlStrict
			&& !self::UpgradeDatabaseStructureStep(
				StructureVersion::MySqlStrict,
				$db,
				$ajax,
